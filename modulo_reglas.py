import os                       # Importamos el modulo OS para interactuar con la consola del sistema. 
import keyboard                 # Importamos el modulo keyboard para interactuar con el teclado.
import colores as estilo_texto  # Importamos el modulo colores para resaltar partes del texto y le ponemos un nombre mas explicito.

def borrar_pantalla():
    """
    Borra la consola cuando precionamos la tecla C
    """
...
def reglas_basicas():
    """
    reglas_basicas: Muestra las reglas basicas del juego.
    """
    print(f"""
        {estilo_texto.subr(estilo_texto.bold("Valor de las cartas:"))}
...
los jugadores.
    ...
    print(f"""   
                {estilo_texto.subr(estilo_texto.bold("Objetivo:"))}
...
puntos que ella, o dejando que su puntuación supere los 21 puntos.
    ...
    print(f"""
        {estilo_texto.subr(estilo_texto.bold("Acciones basicas:"))}
...
""")
    ...
    print(f"""
                {estilo_texto.subr(estilo_texto.bold("Cortador:"))}
...
cantidad en ellas, y jugando de forma separada.
                       ...
                       {estilo_texto.red("[10♥]")}{estilo_texto.black("[10♦]")}
                       apuesta: 100
                            | 
                            |
        {estilo_texto.red("[10♥]")}[8♣]      <<<<<|>>>>>      {estilo_texto.black("[10♦]")}[5♣]
...
""")
    ...
    print(f"""
{estilo_texto.subr("Seguro:")}

Si la carta de la banca es un AS, el jugador puede asegurarse a si 
mismo contra un blackjack, pagando la mitad de su apuesta inicial.
...
def reglamento_completo():
    ...
    print(f"""
                            {estilo_texto.subr(estilo_texto.bold("Básicos del blackjack"))}
...
conseguir un as y una carta con valor de 10 para llegar a una puntuación total de 21.""")
    ...
    print(f"""
                                ...
                                {estilo_texto.subr(estilo_texto.bold("Apuestas y ganancias"))}
...
le devolverá la cantidad inicial que apostó.""")
    ...
    print(f"""
                                  ...
                                  {estilo_texto.subr(estilo_texto.bold("Límites de mesa"))}
...
tengan sus cartas, el crupier será el último en actuar en el juego de blackjack.""")
    ...
    print(f"""
                                    ...
                                    {estilo_texto.subr(estilo_texto.bold("Modo de juego"))}
...
blackjack de las que hemos hablado aquí no cambiarán.
                                    ...
                                    {estilo_texto.subr(estilo_texto.bold("¡Listo para jugar y ganar!"))}
""")
    borrar_pantalla()