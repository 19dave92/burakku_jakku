...
class Jugador():
    ...
    def apuesta(self):
        while True:
            ...
            else: # <--- Se ejecuta si pasó el try
    ...
    def mano(self, hand):
        ...
        #Almacena la mano de cada jugador
        self.hand = hand
        ...
        elif (vm.contiene_AS(self.hand) == True):
            print(f"Cartas de {self.nombre_jugador}: [{self.hand[0]}][{self.hand[1]}] y suman {sum(self.suma_mano_inicial)-10} / {sum(self.suma_mano_inicial)}")
        else:
            print(f"Cartas de {self.nombre_jugador}: [{self.hand[0]}][{self.hand[1]}] y suman {sum(self.suma_mano_inicial)}")
